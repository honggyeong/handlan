...
- **수학Ⅱ**에서 도함수 개념을 학습하며, 접선의 기울기를 또 다른 함수로 표현할 수 있다는 점에 흥미를 느꼈습니다. 함수의 기울기를 활용하여 물체의 속도와 가속도를 구하는 실생활 문제를 해결하며, 수학적 개념과 실제 응용을 연결하는 경험을 쌓았습니다.
- 『딥러닝을 위한 수학: 신경망 수학 기초부터 역전파와 경사하강법까지(로널드 크노이젤)』를 읽고, 인공지능 학습에서의 **역전파 알고리즘**과 **경사하강법**에 대해 탐구하였습니다. 역전파 알고리즘에서 경사하강법이 함수의 최솟값을 찾기 위해 합성 함수의 미분을 연쇄법칙을 활용해 계산된다는 점을 학습하였고, 합성 함수의 기울기를 구하기 위해 이전 함수들의 미분값을 반복적으로 계산하는 방식이 역전파 알고리즘의 원리임을 이해하며 수학적 사고력과 논리적 추론 능력을 심화하였습니다.
- 미분이 단순히 기울기를 계산하는 수학적 개념을 넘어, 데이터 변화를 예측하고 인공지능 학습에서 가중치를 조정하는 데 활용된다는 점을 깨달았습니다. Relu 함수와 Sigmoid 함수와 같은 활성화 함수에서의 역전파 계산 방법을 학습하며, **LSTM의 활성 함수**로 사용되는 다양한 함수에 대해 심화적으로 탐구하고자 하는 의지를 갖게 되었습니다.
- 이러한 수학적 탐구와 관심이 이번 LSTM 기반 실시간 수어 인식 프로젝트의 직접적인 동기가 되었습니다.
